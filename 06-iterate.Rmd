...
You can have two-dimentional vectors -- matrixes.
Data frames/tibbles consist of vectors bound column wise. 
...
Importantly, tidyverse functions will throw an error if recycling other than scalar:
...
Example of more complicated R object
```{r}
...
For example, when data is not first argument
```{r}
map_lgl(LETTERS, ~grepl("B", .x))
```
...
```{r}
x <- list(list(1, 2, 3), list(4), list(7, 8, 9))
```
...
### How to get tidy summary of your models
...
newdata <- data_grid(gapminder, country, year = 2021:2030) %>% 
  left_join(gdp)
```
...
coefs <- estimates %>% 
  ...
  rename(intercept = `(Intercept)`, b_year = year, b_gdpPercap = gdpPercap)

predictions <- left_join(newdata, coefs, by = "country") %>% 
  mutate(y_hat = intercept + b_year * year + b_gdpPercap * gdpPercap)
```
...
predictions %>% 
  group_by(country) %>% 
  mutate(growth = y_hat[year == 2030] - y_hat[year == 2021]) %>% 
  ggplot(aes(x = year, y = y_hat)) +
  geom_line(aes(group = country, color = b_year < 0, linetype = growth == max(growth)))
```
